...
BEGIN
  ...
  CASE NEW.status
    ...
    WHEN 'CANCELADA' THEN
      ...
      v_message := 'Implementar logs detalhados e um histórico auditável de cada estorno de reservas (valor, motivo, usuário, oferta e transação) para facilitar auditoria no admin. Adicionar preferências de notificação no perfil do usuário para escolher receber cancelamentos por e-mail e/ou WhatsApp. Criar um endpoint GET para o admin consultar estornos por oferta e por período, retornando status e valores envolvidos.';
  END CASE;
