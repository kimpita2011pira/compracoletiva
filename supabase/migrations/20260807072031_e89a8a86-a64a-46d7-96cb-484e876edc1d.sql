...
BEGIN
  ...
  CASE NEW.status
    ...
    WHEN 'CANCELADA' THEN
      ...
      v_message := 'Implementar as rotinas de reconciliação mencionadas na mensagem. Desenvolver os testes automatizados para o fluxo de cancelamento. Implementar a rotina de reconciliação periódica da carteira para detectar divergências entre reservas, débitos e créditos e corrigir automaticamente quando houver inconsistências.';
  END CASE;
