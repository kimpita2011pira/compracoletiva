...
BEGIN
  ...
  CASE NEW.status
    ...
    WHEN 'CANCELADA' THEN
      ...
      v_message := 'Desenvolver testes automatizados cobrindo o fluxo de cancelamento de oferta, incluindo estorno, criação de transações e atualização do saldo do cliente. Adicionar um relatório/auditoria para o admin listar discrepâncias detectadas na carteira e visualizar as correções aplicadas pela reconciliação.';
  END CASE;
