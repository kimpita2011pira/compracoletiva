...
BEGIN
  ...
  CASE NEW.status
    ...
    WHEN 'CANCELADA' THEN
      ...
      v_message := 'corrigir movimentação de valores das ofertas: valor da oferta ser reservado da carteira do usuário comprador e ser devolvido imediatamente após a finalização da oferta caso não seja efetivada, e o valor da reserva ser enviada ao vendedor imediatamente após a efetivação da oferta (descontados os 10% da plataforma que deverão ser adicionados a carteira do adm).';
  END CASE;
